...
import {
	LEGACY_STATE_CHECKPOINT_FORMAT,
...
test("Checkpoint akzeptiert nur einen lückenlosen, zeitlich nachfolgenden Event-Tail", () => {
	...
	assert.deepEqual(Object.keys(checkpoint.state), [...STATE_CHECKPOINT_CORE_KEYS]);
	assert.equal(Object.hasOwn(checkpoint.state, "heftBlobs"), false);
...
test("v1-Checkpoint bleibt für die verlustfreie Migration lesbar", () => {
	const base = [{ seq: 1, id: "legacy", t: "2026-08-26T10:00:00.000Z" }];
	const checkpoint = {
		format: LEGACY_STATE_CHECKPOINT_FORMAT,
		maxSeq: 1,
		eventCount: 1,
		lastEventId: "legacy",
		maxTime: base[0].t,
	...
	assert.equal(isUsableStateCheckpoint(checkpoint, { count: 1, maxSeq: 1 }, base[0], []), true);
});
